...
def replace_memory(pid, heap_start, heap_end, search_string, rep_string):
    ...
    try:
        with open("/proc/{:d}/mem".format(pid), "r+b") as file:
            ...
            heapy = file.read(heap_end - heap_start)
            position = heapy.find(search_string.encode())
            if position > -1:
                location = heap_start + position
                print("Found string address!")
                file.seek(location)
                print("Now replacing the string...")
                written = file.write(rep_string.encode() + b'\x00')
                print("Wrote {:d} bytes".format(written))
            else:
...
if __name__ == "__main__":
    ...
    Usage ./read_write_heap.py <PID> <search string> <replacement string>
    """
